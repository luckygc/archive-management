...
class ArchiveDynamicTableNamesTests {
    ...
    void shouldUseSeparateDefaultTablesForMetadataAndPhysicalFields() {
        ...
        assertThat(
                ...
                .isEqualTo("am_archive_record_volume_contract_800c327aefb3");
        assertThat(
                        ArchiveDynamicTableNames.tableName(
                                category, ArchiveLevel.volume, ArchiveFieldScope.physical))
                .isEqualTo("am_archive_record_volume_physical_contract_800c327aefb3");
        assertThat(
                        ArchiveDynamicTableNames.tableName(
                                category, ArchiveLevel.item, ArchiveFieldScope.metadata))
                .isEqualTo("am_archive_record_item_contract_800c327aefb3");
        assertThat(
                        ArchiveDynamicTableNames.tableName(
                                category, ArchiveLevel.item, ArchiveFieldScope.physical))
                .isEqualTo("am_archive_record_item_physical_contract_800c327aefb3");
    }
    ...
    void shouldKeepGeneratedNamesStableAndWithinPostgreSqlIdentifierLimit() {
        ...
        String first =
                ArchiveDynamicTableNames.stableIdentifier("am_archive_record_item_", stableKey);
        String second =
                ArchiveDynamicTableNames.stableIdentifier("am_archive_record_item_", stableKey);

        assertThat(first).isEqualTo(second);
        assertThat(first).hasSizeLessThanOrEqualTo(63);
        assertThat(first).startsWith("am_archive_record_item_very_long_archive_category");
        assertThat(first).endsWith("_66ff6755ef6d");
    }

    @Test
    void shouldAvoidCollisionsWhenReadableFragmentsAreEqual() {
        String first = ArchiveDynamicTableNames.stableIdentifier("am_archive_record_item_", "A-B");
        String second = ArchiveDynamicTableNames.stableIdentifier("am_archive_record_item_", "A_B");

        assertThat(first).startsWith("am_archive_record_item_a_b_");
        assertThat(second).startsWith("am_archive_record_item_a_b_");
