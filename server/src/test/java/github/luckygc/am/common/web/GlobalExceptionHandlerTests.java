...
class GlobalExceptionHandlerTests {
    ...
    void responseStatusExceptionUsesGoogleApiErrorShape() {
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/v1/archive-records/1");
        MDC.put(TraceIdFilter.TRACE_ID, "trace-20260622");
        try {
            var response = handler.handleResponseStatusException(
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "档案记录不存在"),
                    request);
            ...
            assertThat(response.getBody().error().details()).singleElement().satisfies(detail -> {
                assertThat(detail).containsEntry("@type", "type.googleapis.com/google.rpc.ErrorInfo");
                assertThat(detail).containsEntry("reason", "NOT_FOUND_ERROR");
                assertThat(detail).containsEntry("domain", "archive-management");
                assertThat(detail.get("metadata")).asString().contains("trace-20260622");
            });
        } finally {
    ...
    void badRequestExceptionIncludesFieldViolations() {
        MockHttpServletRequest request = new MockHttpServletRequest("POST", "/api/v1/archive-records");
