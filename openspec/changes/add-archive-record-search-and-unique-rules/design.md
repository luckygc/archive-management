...
首版字段来源只支持案卷或卷内电子字段，即 `field_scope=metadata` 且与唯一规则 `archive_level` 一致的分类字段；实物信息字段不参与唯一校验。需要范围唯一时，范围字段也必须是分类电子字段，例如“部门 + 合同编号”联合唯一。空值语义沿用 PostgreSQL 默认唯一索引行为，即 `NULL` 不互相冲突；如果未来要求空值也冲突，再引入表达式索引或应用层归一化规则。规则新增、启用或字段组合变化时创建或重建对应索引；规则禁用或删除时删除对应索引。
