...
- **AND** 系统 SHALL 拒绝同时提交全宗、分类、密级、保管期限、所属组织机构或动态字段条件
- **AND** 该范围在查询编译时 SHALL 表达为任意档案数据

#### Scenario: 创建条件范围

- **WHEN** 管理员创建条件数据范围
- **THEN** 系统 SHALL 至少保存一个全宗、分类、密级、保管期限、所属组织机构或动态字段条件
...
系统 SHALL 支持全宗、分类、密级、保管期限和档案所属组织机构等固定维度数据范围条件，并支持树形维度是否包含子级。
...
- **WHEN** 管理员选择保管期限范围
- **THEN** 系统 SHALL 校验保管期限 ID 来自固定保管期限字典
- **AND** 查询编译时 SHALL 将保管期限 ID 条件作为档案主表固定字段谓词的一部分

#### Scenario: 保存档案所属组织机构范围

- **WHEN** 管理员选择档案所属组织机构范围
- **THEN** 系统 SHALL 校验组织机构存在且启用
- **AND** 查询编译时 SHALL 将组织机构 ID 条件应用到档案主表 `org_unit_id`
- **AND** 该维度 SHALL 表达档案记录所属组织机构，不等同于授权主体绑定中的组织类主体预留
...
- **AND** 在组织主体继承计算能力未实现前，系统 SHALL NOT 将组织主体绑定计入用户有效数据范围
- **AND** 组织主体绑定 SHALL NOT 与档案记录所属组织机构固定维度混用
...
#### Scenario: 创建或更新档案时校验所属组织机构范围

- **WHEN** 用户创建或更新档案并提交所属组织机构 ID
- **THEN** 系统 SHALL 校验组织机构存在且启用
- **AND** 系统 SHALL 使用提交后的全宗、分类、密级、保管期限和所属组织机构判断档案是否命中用户数据范围
