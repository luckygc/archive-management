...
系统 SHALL 支持后台管理场景按分类、全宗、固定字段、所属组织机构、动态字段和档案状态查询档案记录，并在后端应用当前用户档案数据范围。
...
#### Scenario: 按所属组织机构查询档案

- **WHEN** 用户按所属组织机构筛选档案管理列表
- **THEN** 系统 SHALL 将组织机构筛选作为用户查询条件的一部分
- **AND** 系统 SHALL 继续叠加当前用户档案数据范围
- **AND** 系统 SHALL NOT 因用户提交组织机构筛选而放宽已有数据范围
