...
本变更横跨 Vue PC、档案 item 子域和 MyBatis 动态表查询。实现必须遵守现有 `api-contract`、`archive-metadata`、`archive-record-search`，继续以服务端权限、数据范围和状态约束为真相源，并保持 Element Plus 高密度企业工作台的既有设计语言。
...
案卷页面复用 `/api/v1/archive-volumes` 的列表、创建、详情与 `:addItem` 动作。查看卷内档案通过档案搜索的 `volumeId` 固定字段筛选完成，避免再造一套档案列表响应。关系维护复用 `/api/v1/archive-items/{archiveItem}/relations` 子资源，目标档案从现有游标搜索结果中选择，禁止用户手输数据库 ID。
...
列表使用统一游标响应，`limit`、`cursor` 通过 URL query 提交；写请求使用动作明确的 `CreateArchiveItemLineRowRequest`、`UpdateArchiveItemLineRowRequest`，响应使用 `ArchiveItemLineRowResponse`。Service 先校验档案数据范围、更新权限、档案可编辑状态、明细表已构建且属于档案分类，再把字段编码映射为元数据白名单列。动态表名和列名只能来自已验证定义，请求键和值不得直接拼接 SQL；MyBatis 负责动态行查询与写入，删除沿用逻辑删除字段。
...
1. 先补充明细行和工作台摘要服务端合同、测试与动态 SQL，不改变现有数据库结构。
2. 接通 PC API、页面、权限与错误状态；服务端先部署时旧 Web 不受影响。
