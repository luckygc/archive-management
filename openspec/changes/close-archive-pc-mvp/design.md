...
权限 Store 提供唯一 `has(code)` 判断：`superAdmin` 直接返回 true，普通用户检查服务端返回的权限码集合。初始化状态、超级管理员标记、权限码和递增版本组成一个原子快照；只有最新请求成功才整体替换，刷新失败保留上一成功快照，重置同时使在途响应失效。路由元数据通过 `permission` 声明单一权限，通过 `permissionsAnyOf` 声明满足任意一个即可访问的能力聚合页；菜单叶子、空分组、页签、按钮和导航守卫复用同一判断。权限撤销时先隐藏当前缓存内容并清理其他无权页签，当前页签只在最新 403 导航成功后删除；导航取消、拒绝或过期时保留页签并使用内联 403 收敛界面。
