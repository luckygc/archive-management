...
权限 Store 提供唯一 `has(code)` 判断：`superAdmin` 直接返回 true，普通用户检查服务端返回的权限码集合。路由元数据通过 `permission` 声明单一权限，通过 `permissionsAnyOf` 声明满足任意一个即可访问的能力聚合页；菜单叶子、空分组、页签、按钮和导航守卫复用同一判断。权限摘要在会话中刷新并撤销权限后，PC 立即移除无权页签及其缓存，当前页面无权时进入统一无权限页。

授权管理页只聚合 `authorization:permission:manage` 和 `archive:data-scope:manage` 两项能力：前者只维护角色功能权限，不请求数据范围接口；后者只维护角色、用户和部门的数据范围，不请求功能权限接口。角色目录允许角色、用户、功能权限或数据范围管理员读取，用户目录允许用户或数据范围管理员读取；目录写入与详情接口继续使用各自原有精确权限。服务端仍对每次敏感读写执行权限、数据范围和状态校验。

这比在菜单、页面和按钮中分别维护权限集合更少状态源，也避免仅靠隐藏菜单保护路由。目录读取采用显式最小 OR 权限，写入不随页面聚合而放宽。
