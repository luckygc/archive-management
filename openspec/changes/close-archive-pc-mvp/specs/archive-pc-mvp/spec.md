...
- **WHEN** 已初始化的权限摘要刷新后不再包含当前页面或已打开页签所需权限
- **THEN** PC SHALL 移除所有无权页签及其缓存内容
- **AND** 当前页面无权时导航守卫 SHALL 将用户带到统一无权限页面
...
- **THEN** 授权管理 SHALL 只加载角色、用户、部门及数据范围目录和主体范围
- **AND** 页面 SHALL NOT 请求功能权限接口
- **AND** 服务端 SHALL 仅为目录读取接受明确的相关权限任一项，详情和写入 SHALL 继续要求各自精确管理权限
