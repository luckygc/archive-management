...
系统 SHALL 允许管理员直接定义带自身作用域的结构化约束和规则。
...
- **AND** 编码 SHALL 在全部未删除运行时定义中唯一
- **AND** 请求和响应 SHALL NOT 包含额外的方案或版本容器 ID
...
- **AND** 请求 SHALL NOT 要求额外的方案或版本容器 ID
...
系统 SHALL 在业务持久化前以单次稳定顺序执行与当前对象作用域匹配的运行时规则和约束。

#### Scenario: 执行运行时配置

- **WHEN** 业务 Service 到达固定触发点
- **THEN** 系统 SHALL 按触发点、全宗、分类和档案层级加载已发布启用定义
...
- **THEN** 系统 SHALL 保存触发点、对象、定义编码、动作摘要、结果和时间
- **AND** 追踪 SHALL NOT 保存额外的方案或版本容器 ID
- **AND** 追踪 SHALL NOT 保存未脱敏的敏感字段全文
- **AND** 查询 SHALL 先在数据库中应用功能权限和档案数据范围，再按 `created_at DESC, id DESC` 使用统一 cursor 分页

### Requirement: 独立运行时规则工作区

系统 SHALL 提供直接维护运行时约束和规则的工作区。

#### Scenario: 打开规则工作区

- **WHEN** 管理员进入运行时规则页面
- **THEN** 页面 SHALL 直接查询运行时定义
- **AND** 页面 SHALL NOT 要求先选择额外的方案或版本容器
- **AND** 页面 SHALL 提供定义类型、固定触发点、作用域、真实字段条件、优先级和固定动作参数编辑能力

#### Scenario: 查询运行时追踪

- **WHEN** 管理员进入运行时追踪页面
- **THEN** 页面 SHALL 支持按触发点、对象和结果筛选
- **AND** 页面 SHALL NOT 展示或筛选额外的方案或版本容器
