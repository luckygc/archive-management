...
系统 SHALL 支持编辑单条档案条目的现有固定字段、案卷归属、未正式归档条目的全宗和动态字段；正式归档后的跨全宗变化 SHALL 使用受控调整动作。
...
#### Scenario: 普通编辑纠正未归档条目全宗

- **WHEN** 客户端通过普通 PATCH 修改尚无正式归档时间且未锁定条目的全宗
- **THEN** 系统 SHALL 按普通编辑流程校验目标全宗、分类范围、权限、数据范围和案卷一致性
- **AND** 校验通过后 SHALL 更新条目全宗

#### Scenario: 普通编辑拒绝正式归档条目跨全宗

- **WHEN** 客户端通过普通 PATCH 修改已有正式归档时间条目的全宗
- **THEN** 系统 SHALL 拒绝更新并返回前置条件错误
- **AND** 响应 SHALL 指出应使用调整全宗动作
- **AND** 条目、动态字段、搜索投影和审计 SHALL 保持不变

#### Scenario: 受控调整正式归档条目全宗
...
- **AND** 系统 SHALL 更新条目全宗并写入 `REASSIGN_FONDS` 操作审计
- **AND** 审计原因 SHALL 包含原全宗、目标全宗和客户端提交的原因
- **AND** 系统 SHALL 返回更新后的档案条目详情

#### Scenario: 调整全宗前置条件不满足

- **WHEN** 调整目标全宗已封闭、未配置当前分类、超出数据范围，或条目已锁定、已归入案卷
- **THEN** 系统 SHALL 拒绝调整
- **AND** 条目全宗、搜索投影和审计 SHALL 保持不变
