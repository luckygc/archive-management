...
1. 后端是单 Spring Boot 应用；`app` 只放启动和应用级装配，业务放 `module`，技术适配放 `infrastructure`，`common` 只放跨业务基础约定，不放业务语义或外部技术适配。
...
9. 后端 Maven 根目录是 `server/`；前后端具体命令以 `Taskfile.yml` 和构建配置为准，不在本文件复制命令矩阵。
