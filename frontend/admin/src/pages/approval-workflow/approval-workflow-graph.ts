...
export function createDefaultApprovalGraph(): ApprovalWorkflowGraphDto {
    return {
        ...
        edges: [
            edge("flow_start_approval_1", "start", "approval_1"),
            edge("flow_approval_1_end", "approval_1", "end"),
        ],
    };
}
...
export function fromLogicFlowGraph(data: LogicFlowGraphData): ApprovalWorkflowGraphDto {
    ...
    return {
        nodes: data.nodes.map((item) => {
            ...
            const nodeType =
                reverseNodeTypeMap[item.type] ?? (properties.nodeType as ApprovalNodeType);
            return {
                nodeCode: item.id,
                ...
                candidateUserIds:
                    nodeType === "APPROVAL" ? numberArray(properties.candidateUserIds) : [],
...
export function validateApprovalGraph(graph: ApprovalWorkflowGraphDto): GraphIssue[] {
    ...
    for (const item of graph.nodes) {
        if (!item.nodeName.trim())
            issues.push({ elementId: item.nodeCode, message: "节点名称不能为空" });
        if (item.nodeType === "APPROVAL" && item.candidateUserIds.length === 0) {
            issues.push({
                elementId: item.nodeCode,
                message: `${item.nodeName}：请选择至少一名候选用户`,
            });
        }
        const incoming = graph.edges.filter(
            (edgeItem) => edgeItem.targetNodeCode === item.nodeCode,
        );
        const outgoing = graph.edges.filter(
            (edgeItem) => edgeItem.sourceNodeCode === item.nodeCode,
        );
        ...
        if (item.nodeType === "APPROVAL" && outgoing.length !== 1)
            issues.push({
                elementId: item.nodeCode,
                message: `${item.nodeName}：必须且只能有一条出线`,
            });
        if (item.nodeType === "EXCLUSIVE_GATEWAY") {
            ...
            if (outgoing.length < 2)
                issues.push({
                    elementId: item.nodeCode,
                    message: `${item.nodeName}：至少需要两个分支`,
                });
            if (defaults.length !== 1)
                issues.push({
                    elementId: item.nodeCode,
                    message: `${item.nodeName}：必须设置一个默认分支`,
                });
        }
    }
    ...
    for (const item of graph.edges) {
        ...
        if (source?.nodeType === "EXCLUSIVE_GATEWAY" && !item.defaultFlow) {
            if (
                ...
                issues.push({
                    elementId: item.edgeCode,
                    message: "等于和不等于条件只能填写一个值",
                });
            }
        }
    }
    if (starts[0]) {
        ...
        for (const item of graph.nodes.filter((nodeItem) => !reachable.has(nodeItem.nodeCode))) {
            issues.push({
                elementId: item.nodeCode,
                message: `${item.nodeName}：无法从开始节点到达`,
            });
        }
    ...
    if (ends[0]) {
        ...
        for (const item of graph.nodes.filter((nodeItem) => !reachesEnd.has(nodeItem.nodeCode))) {
            issues.push({
                elementId: item.nodeCode,
                message: `${item.nodeName}：无法到达结束节点`,
            });
        }
    }
    return issues;
