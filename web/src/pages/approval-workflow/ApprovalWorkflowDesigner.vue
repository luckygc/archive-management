...
defineExpose({ addNode, startDrag, deleteSelected, updateNode, updateEdge, focusElement, undo, redo, zoomIn, zoomOut, fitView });
...
onMounted(() => {
    ...
    instance.setTheme({
        polyline: { stroke: "#64748b", strokeWidth: 1.5, hoverStroke: "#2563eb", selectedStroke: "#2563eb" },
    ...
    instance.on("edge:add", ({ data }) => {
        ...
        if (data.sourceNodeId === data.targetNodeId || (duplicates?.length ?? 0) > 1) {
            ...
            ElMessage.warning(data.sourceNodeId === data.targetNodeId ? "节点不能连接自身" : "相同节点之间不能重复连线");
        }
        emitChange();
    });
