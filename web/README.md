...
非登录页会先校验 session；后端返回 401 时跳转到 `/login?redirect=...`。登录页使用 `cap-widget` 获取 `powToken`，浏览器端通过 `CAP_CUSTOM_FETCH` 将 widget 内部请求改写到项目 REST CAP API，登录成功后返回 redirect 指向的业务页面。请求层遇到 401 会广播未登录事件，路由守卫清理会话和页签状态。
