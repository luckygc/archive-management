...
- `axios`：由 `frontend-core` 封装统一 API client。
...
菜单、面包屑、页签标题和缓存策略统一来自 `web/src/app/routes.ts` 的路由树。菜单递归渲染路由树，不限制二级、三级或四级；面包屑直接使用 `RouterView` 提供的匹配路由链。业务路由默认进入缓存；只有明确设置 `meta.cache = false` 的页面不缓存，登录、认证错误等公开路由通过 `meta.menu = false` 或位于工作区路由树之外而不生成菜单。
