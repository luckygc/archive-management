...
当前系统已经存在 `am_organization_unit`、`OrganizationUnit` 和 `/api/v1/organization-units` 只读接口，并且档案数据范围、档案条目写入已经引用组织单元。但该模型命名偏抽象，`unit` 不符合系统管理里的真实业务语言；同时前端没有组织架构管理页，用户没有所属部门，数据范围中预留的组织主体也没有参与用户权限计算。
...
- 提供组织架构管理页，支持部门树维护。
- 用户管理支持所属部门。
- 档案数据范围支持部门维度，并让用户所属部门范围参与运行时数据范围计算。
...
- `am_archive_item.org_unit_id` 改为 `department_id`。
- `am_archive_volume.org_unit_id` 改为 `department_id`。
- `am_authentication_user` 新增 `department_id`，允许为空。
- 档案数据范围维度枚举从 `ORG_UNIT` 改为 `DEPARTMENT`，维度仍通过 `target_id` 指向部门 ID。

迁移处理：

- 因项目未正式发布，Flyway 迁移直接维护目标结构。
- 现有 `V20260703_0100__create_organization_unit.sql` 调整为创建 `am_organization_department`，并同步更新 archive item / volume 的列名、外键和注释。
- 不写旧结构迁移兼容分支。
...
- `archive.authorization` 通过 `OrganizationDepartmentService` 校验部门范围条件。
- `archive.item` 通过 `OrganizationDepartmentService` 校验档案写入里的部门。
...
- 超级管理员默认拥有。
- 组织架构管理接口检查该权限。
- 数据范围页读取启用部门选项时，后端允许具备 `archive:data-scope:manage` 的调用方读取部门列表，避免数据范围配置绕到组织管理权限。
...
- “组织单元范围”改为“部门范围”。
- 前端枚举从 `ORG_UNIT` 改为 `DEPARTMENT`。
- 选项使用启用部门扁平列表，label 为 `部门编码 部门名称`。
...
- 超级管理员仍固定拥有全部数据范围。
- 档案记录自身的 `department_id` 仍用于数据范围维度过滤，表达“哪些部门形成或维护的档案可见”。
...
- 设置用户所属部门时，部门必须存在且启用。
- 档案写入设置部门时，部门必须存在且启用。
- 数据范围设置部门条件时，部门必须存在且启用。
- 停用部门允许存在历史引用；停用后不能被新用户、新档案记录、新数据范围条件选择。
...
- 用户管理部门列和部门选择。
- 数据范围维度从 `ORG_UNIT` 改为 `DEPARTMENT`。
...
3. 改造认证用户模型与用户管理接口。
4. 改造档案数据范围和档案写入中的 `ORG_UNIT` / `orgUnitId`。
