...
- [运行时约束、规则与治理方案](./runtime-policies-and-governance.md)：真实字段目录、固定触发点与动作、事务边界及动态配置快照。
...
5. 如果目标是多机构运行时治理，读 [运行时约束、规则与治理方案](./runtime-policies-and-governance.md)，确认字段、规则、事务和迁移恢复边界。
...
- 治理方案、运行时约束、固定动作和配置快照已进入 OpenSpec 合同；后续以 `archive-governance-scheme`、`archive-local-rule-engine` 和 `archive-runtime-check-portability` 规格为准。
