...
- [运行时约束与规则](./runtime-policies.md)：真实字段目录、固定触发点与动作及事务边界。
...
5. 如果目标是配置运行时检查，读 [运行时约束与规则](./runtime-policies.md)，确认字段、规则和事务边界。
...
- 运行时约束和固定动作已进入 OpenSpec 合同；后续以 `archive-local-rule-engine` 规格为准。
