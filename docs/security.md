...
- PostgreSQL、外部 S3 兼容对象存储和部署平台是独立运行边界，使用最小权限账号和受控网络访问。
...
安全配置变更后运行 `mise run governance-check` 和与改动范围匹配的后端测试，并在预生产验证认证失败、权限不足、数据范围、CSRF、CORS、签名重放、短链过期和 Actuator 隔离路径。
