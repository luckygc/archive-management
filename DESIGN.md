...
colors:
    ...
    primary-ant:
        value: "#1677ff"
        role: Ant Design 默认主色兼容色
    background:
        value: "#f5f7fb"
        role: PC 工作区和移动端页面背景
    surface:
        value: "#ffffff"
        role: 菜单、顶部栏、表格容器、表单区域和移动端信息块
    text:
        value: "#172033"
        role: 主要文本
    text-secondary:
...
Archive Management 的设计系统服务于长期、高频、可审计的档案业务操作。它以 Ant Design 的企业级产品语言为主线，强调稳定结构、明确状态和可预期反馈，而不是营销展示、装饰叙事或炫技视觉。

PC 端是高密度工作台：侧边菜单、多页签、表格、筛选、动态表单和详情区是主要表达。移动端是轻门户：只保留待办、审批、归档接收入口和轻量查询，不把 PC 管理界面压缩复制到手机上。

系统明确拒绝营销落地页、宣传站、深色炫技后台、大面积渐变、玻璃拟态、装饰卡片、插画堆叠和品牌 hero。组件外观优先交给 Ant Design / Ant Design Mobile，不另起一套平行组件库。
...
- PC 和移动端共享品牌色、字体和登录能力，但交互密度分开设计。
...
- **Ant Blue** (#1677ff): 保留给 Ant Design 默认组件状态和图标兼容场景，避免在局部覆盖组件库状态色。

### Neutral

- **Workbench Mist** (#f5f7fb): PC 内容区和移动端页面背景。它负责把工作区和白色控件表面区分开。
- **Panel White** (#ffffff): 菜单、顶部栏、表格容器、表单区域、移动端摘要块和弹层表面。
...
- **Display** (600, 24-28px, 1.2): 只用于登录页或移动端入口标题。
...
- **Component Default Shadow** (`Ant Design default`): 弹层和浮动反馈使用组件库默认阴影，不额外重写。
- **Flat Surface** (`none`): 表格容器、筛选区、表单区和移动端摘要块默认无阴影，用边框和背景区分。
...
组件应直接继承 Ant Design / Ant Design Mobile 的行为、状态和可访问性。自定义样式只负责布局、尺寸、滚动区域和必要间距。

### Buttons

- **Shape:** PC 默认 6px 圆角，移动端跟随 Ant Design Mobile。
- **Primary:** `#1f5eff` 或 Ant Design token `colorPrimary`，用于页面主动作和确认动作。
- **Hover / Focus:** 使用组件库默认状态；图标按钮必须有可访问名称。
- **Secondary / Ghost / Tertiary:** 使用组件库默认按钮类型，不自绘新按钮。

### Chips

- **Style:** 状态和分类优先用 `Tag`；筛选型选择可用 `Segmented`、`Tabs` 或移动端列表项。
...
- **Internal Padding:** PC 内容区 16px；移动端内容区 12px。
...
PC 使用左侧 `Menu`、顶部用户区和多页签表达结构。移动端使用 `NavBar`、`List` 和轻量入口表达任务，不复刻 PC 侧边栏、多页签和复杂筛选。

## 6. Do's and Don'ts

### Do:

- **Do** 通过 `ConfigProvider`、Ant Design token 和少量布局 CSS 控制主题。
- **Do** 让首屏直接呈现表格、筛选、表单、详情、状态面板或待办内容。
- **Do** 覆盖 Loading、Empty、Error、Disabled、Submitting 等状态。
- **Do** 保持 PC 管理界面高密度，移动端低密度。
