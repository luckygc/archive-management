...
产品定位、用户和产品原则见 [`PRODUCT.md`](PRODUCT.md)；本文只维护 Element Plus 当前设计系统。
...
系统以边界线、背景分层和组件库弹层阴影表达层级。常驻工作区表面尽量保持平，阴影只用于 Dialog、Drawer、Dropdown、Tooltip、Popover 等浮层或组件库已有状态。
...
- **其他按钮层级:** 按业务语义使用 Element Plus 公开的 default、plain、text 或 link 能力，不自绘新按钮类型。
...
- **Focus:** 使用组件库默认 focus ring；禁止依赖或覆盖 Element Plus 及第三方组件的内部 DOM 类或结构，优先使用 CSS 变量、组件 props 和公开扩展点。
...
- **Don't** 依赖或覆盖 Element Plus 及第三方组件的内部 DOM 类或结构来改变基础视觉。
